...
def process_file(uploaded_file):
    global global_result_df  # Declare global to modify the global variable
    import os
    ...
    master_name_df = pd.read_excel('D:\master.xlsx', sheet_name='name')
    master_category_df = pd.read_excel('D:\master.xlsx', sheet_name='category')
    master_lob_df = pd.read_excel('D:\master.xlsx', sheet_name='lob', header=None)
    ...
    global_result_df = result
    return result


# result = transform('D:\input1.xlsx')
...
def display_plot(request):
    ...
    if global_result_df is not None:
        ...
        plt.bar(global_result_df['Product'], global_result_df['value'], color='skyblue')
        plt.xlabel('Product')
        plt.ylabel('Value')
...
def download_output(request):
    ...
    if global_result_df is not None:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            global_result_df.to_excel(writer, index=False)
        output.seek(0)
